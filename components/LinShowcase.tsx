...
import { linProducts } from "@/data/linProducts";
import algeriaData from "@/data/algeria.json";
import Image from "next/image";

interface LinShowcaseProps {
...
const LinShowcase: React.FC<LinShowcaseProps> = ({ 
  initialSinglePrice, 
  initialBundlePrice, 
  initialZonePrices 
}) => {
  ...
  const jacket = linProducts[currentIndex];
  ...
  const sendAbandonedLead = useCallback(() => {
    ...
    const payload = JSON.stringify({
      ...
      item: linProducts[currentIndex]?.name,
      color: linProducts[currentIndex]?.colorName,
  ...
  const navigate = useCallback((dir: number) => {
    ...
    setCurrentIndex((prev) => {
      const next = prev + dir;
      if (next < 0) return linProducts.length - 1;
      if (next >= linProducts.length) return 0;
      return next;
    });
    setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <header className="hidden lg:flex relative z-30 justify-between items-center px-10 py-8 shrink-0 w-full">
        ...
        <div className="flex items-center bg-white/10 backdrop-blur-md rounded-full px-5 py-2.5 gap-3 border border-[#b5a898] shadow-xl shadow-[#2c2418]/10">
      ...
      <div className="flex flex-col lg:hidden flex-1 px-4 pt-3 pb-6 gap-4 overflow-y-auto no-scrollbar">
        ...
        <div className="relative w-full aspect-[4/5] rounded-3xl overflow-hidden bg-white/5 border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex-shrink-0 z-40">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2 pointer-events-none z-50">
             ...
             <div className="bg-white/10 backdrop-blur-md rounded-full px-3 py-1.5 border border-white/20 w-fit">
        ...
        <div className="flex flex-col flex-1 justify-between gap-4 mt-1">
           ...
           <div className="flex justify-between items-start z-30">
             <div className="flex flex-col">
               ...
               <div className="flex items-center gap-1 mt-1.5">
                 ...
                 <span className="text-white/50 text-[10px] ml-1 font-medium" style={{ fontFamily: "var(--font-dm)" }}>4.8 (120+ avis)</span>
               </div>
             </div>
             ...
             <AnimatePresence mode="wait">
                ...
                >
                  {selectedQuantity === 2 && (
                    <span className="text-white/50 text-[0.85rem] line-through" style={{ fontFamily: "var(--font-dm)" }}>{(singlePrice * 2).toLocaleString('en')} DA</span>
                  )}
           ...
           <AnimatePresence mode="popLayout">
             <div className="flex flex-col gap-3">
                 ...
                 <div className="flex justify-between items-center bg-white/5 border border-white/20 rounded-[1.2rem] p-3.5 shadow-inner backdrop-blur-sm">
                   <span className="text-white/60 text-[11px] uppercase tracking-widest font-bold font-dm">Size</span>
                   <div className="flex gap-2">
                      {["M", "L", "XL"].map((s) => (
                         <button key={s} type="button" onClick={() => setSelectedSize(s)} className={`w-10 h-10 rounded-xl flex items-center justify-center text-[13px] font-black transition-all duration-300 ${selectedSize === s ? "bg-white text-black shadow-[0_0_15px_rgba(255,255,255,0.3)]" : "bg-white/10 text-white"}`} style={{ fontFamily: "var(--font-dm)" }}>
                           {s}
                         </button>
                      ))}
                   </div>
                 ...
                 <div className="flex flex-col gap-2">
                   ...
                   <div className="flex gap-2">
                      <button type="button" onClick={() => setSelectedQuantity(1)} className={`flex-1 flex flex-col items-center justify-center gap-0.5 rounded-[1.2rem] p-3 transition-all duration-300 border ${selectedQuantity === 1 ? "bg-white text-black border-white shadow-[0_0_20px_rgba(255,255,255,0.2)]" : "bg-white/5 text-white border-white/20"}`}>
               ...
               >
                 <div className="bg-white/10 p-5 rounded-3xl backdrop-blur-xl border border-white/20 mt-1 shadow-2xl flex flex-col gap-3">
                   ...
                   <input required name="name" placeholder="الاسم الكامل" onChange={e => { formNameRef.current = e.target.value; }} className="bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-[15px] text-white placeholder-[#8a7a68] focus:outline-none focus:border-white/50 transition-colors" />
                   <input required type="tel" name="phone" placeholder="رقم الهاتف" onChange={e => { formPhoneRef.current = e.target.value; }} className="bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-[15px] text-white placeholder-[#8a7a68] focus:outline-none focus:border-white/50 transition-colors text-right" dir="ltr" />
                   <div className="flex flex-col gap-2">
                     <select 
                       required 
                       ...
                       className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-[15px] text-white focus:outline-none focus:border-white/50 transition-colors appearance-none"
                     >
                     ...
                     <select 
                       ...
                       className="w-full bg-white/5 border border-white/20 rounded-xl px-4 py-2.5 text-[15px] text-white focus:outline-none focus:border-white/50 transition-colors appearance-none disabled:opacity-50"
                     >
                   ...
                   {/* Order Summary */}
                    <div className="bg-white/5 border border-white/20 rounded-xl p-3 flex flex-col gap-1 mt-1 font-sans">
      ...
      <div className="hidden lg:block absolute top-36 left-10 z-20">
        <AnimatePresence mode="wait">
          <motion.span
            ...
            className="inline-block px-4 py-2 rounded-full text-sm font-semibold tracking-wide backdrop-blur-md border border-white/20 bg-white/5"
      ...
      <div className="hidden lg:flex absolute inset-0 items-center justify-center z-10 pointer-events-none">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="relative w-[480px] h-[600px] rounded-[2.5rem] overflow-hidden shadow-[0_40px_80px_rgba(0,0,0,0.7)] border border-white/20 bg-white/5 pointer-events-auto"
          >
            <Image
              src={jacket.image}
              alt={jacket.name}
      ...
      <div className="hidden lg:flex absolute bottom-0 left-0 right-0 z-20 items-end justify-between px-10 pb-8">
        ...
        <div className="flex flex-col items-center gap-3">
          <AnimatePresence mode="wait">
            ...
            >
              {selectedQuantity === 2 && (
                <span className="text-white/50 text-sm line-through" style={{ fontFamily: "var(--font-dm)" }}>{(singlePrice * 2).toLocaleString('en')} DA</span>
              )}
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-white/50 text-xs tabular-nums" style={{ fontFamily: "var(--font-dm)" }}>{currentIndex + 1} / {linProducts.length}</span>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => navigate(1)} className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors">
        ...
        <div className="relative">
          <AnimatePresence mode="wait">
              <motion.form
                ...
                animate={{ opacity: 1, scale: 1, y: 0 }}
                className="absolute bottom-0 right-0 w-[400px] flex flex-col gap-3 bg-white/10 p-6 rounded-[2rem] backdrop-blur-2xl border border-[#b5a898] shadow-[0_30px_60px_rgba(0,0,0,0.6)] z-50 overflow-hidden"
              ...
              >
                ...
                <input required name="name" placeholder="الاسم الكامل" onChange={e => { formNameRef.current = e.target.value; }} className="bg-white/5 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-[#8a7a68] focus:outline-none focus:border-white/50 transition-colors text-base relative z-10" />
                <input required type="tel" name="phone" placeholder="رقم الهاتف" onChange={e => { formPhoneRef.current = e.target.value; }} className="bg-white/5 border border-white/20 rounded-xl px-4 py-3 text-white placeholder-[#8a7a68] focus:outline-none focus:border-white/50 transition-colors text-right text-base relative z-10" dir="ltr" />
                <div className="flex gap-2 relative z-10">
                  <select 
                    ...
                    className="w-[45%] bg-white/5 border border-white/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-white/50 transition-colors appearance-none cursor-pointer text-base"
                  >
                  ...
                  <select 
                    ...
                    onChange={e => setSelectedCommune(e.target.value)}
                    disabled={!selectedWilaya}
                    className="w-[55%] bg-white/5 border border-white/20 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-white/50 transition-colors appearance-none disabled:opacity-50 cursor-pointer text-base"
                  >
                ...
                {/* Order Summary */}
                <div className="bg-white/5 border border-white/20 rounded-xl p-4 flex flex-col gap-2 mt-1 font-sans relative z-10">
                ...
                <div className="flex gap-3 mt-3 font-sans relative z-10 w-full" dir="ltr">
                  <button disabled={isSubmitting || orderSuccess} type="submit" className="w-full py-4 rounded-xl bg-white text-black font-black uppercase text-sm tracking-wider hover:bg-[#3d3225] transition-transform active:scale-[0.98] shadow-[0_0_20px_rgba(44,36,24,0.3)] cursor-pointer disabled:opacity-75 disabled:scale-100 flex items-center justify-center">
                    {isSubmitting ? "جاري الإرسال..." : orderSuccess ? "تم الطلب بنجاح ✓" : "تأكيد الطلب"}
                  </button>
                </div>
              </motion.form>

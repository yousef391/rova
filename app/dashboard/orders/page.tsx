...
export default function OrdersManagementPage() {
  ...
  const deleteOrder = async (orderId: string, trackingId?: string | null) => {
    ...
    try {
      ...
      console.error(err);
      alert(`Failed to delete order. Error: ${err.message || 'Unknown server error'}`);
    } finally {
      setDeletingId(null);
    }
  };
  ...
  const communesForWilaya = dispatchData.wilaya ? algeriaData.communes.filter((c: any) => c.wilaya_id.toString() === dispatchData.wilaya) : [];
